  ...
  return {
    user: {
      ...
      async create({ data }) {
        ...
        const createdUser = { ...created, id: created._id?.toString?.() || created.id };
        ...
        if (profile?.create) {
          ...
          const profileResult = await profiles.insertOne(profileData);
          const createdProfile = await profiles.findOne({ _id: profileResult.insertedId });
